...
app.get("/random", (req, res) => {
  var randomId = Math.floor(Math.random() * jokes.length);

  res.send(jokes[randomId]);
});
...
app.get("/filter/", (req, res) => {
  ...
  var upperCaseFirstLetter = query.charAt(0).toUpperCase();
  var type = upperCaseFirstLetter + query.substring(1,).toLowerCase();
  var filteredJokes = jokes.filter((joke) => joke.jokeType === type);

  res.send(filteredJokes.length === 0 ? "No joke with the selected type was found" : filteredJokes);
});

app.post("/jokes/", (req, res) => {
  var jokeId = req.body.id;
  var text = req.body.jokeText;
...
app.patch("/joke/:id", (req, res) => {
  var id = req.params.id;
  var text = req.body.jokeText === undefined ? jokes[id].jokeText : req.body.jokeText;
  var type = req.body.jokeType === undefined ? jokes[id].jokeType : req.body.jokeType;
